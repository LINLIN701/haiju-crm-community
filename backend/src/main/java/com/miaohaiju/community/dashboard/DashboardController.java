...
public class DashboardController {
    ...
    ApiResponse<Overview> overview() {
        ...
        Long plannedFollowUps = jdbc.queryForObject("""
                SELECT COUNT(*) FROM contact_records r
                WHERE r.next_contact_at > CURRENT_TIMESTAMP
                  AND r.id = (SELECT MAX(r2.id) FROM contact_records r2 WHERE r2.customer_id = r.customer_id)
                """, Long.class);
        return ApiResponse.ok(new Overview(customerCount == null ? 0 : customerCount,
                ...
                plannedFollowUps == null ? 0 : plannedFollowUps));
    }

    public record Overview(long customerCount, long dueFollowUps, BigDecimal totalConsumption, long plannedFollowUps) {
    }
}
